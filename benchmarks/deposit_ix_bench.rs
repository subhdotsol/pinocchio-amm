...
fn main() {
    ...
    // reserve_x = 0, reserve_y = 0 (bytes 109..125 already zero — first deposit)
    ...
    let user_ata_x_account = make_token_account(&mollusk, mint_x, user, 1_000_000, token_program);
    let user_ata_y_account = make_token_account(&mollusk, mint_y, user, 1_000_000, token_program);
    let user_ata_lp_account = make_token_account(&mollusk, lp_pda, user, 0, token_program);

    let amount: u64 = 100_000;
    let max_x: u64 = 1_000_000;
    let max_y: u64 = 1_000_000;
    let mut data = vec![1u8]; // discriminator = 1 for Deposit
    ...
    MolluskComputeUnitBencher::new(mollusk)
        .bench(("deposit", &instruction, &accounts))
        .must_pass(true)
